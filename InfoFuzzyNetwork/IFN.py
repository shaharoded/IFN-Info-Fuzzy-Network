...
class IFN:
    ...
    def __plot_network(self):
        ...
        for (parent, child, weight) in self.edges:
            ...
            if weight is not None:
                ...
                G.add_edge(parent_id, child_id)
                edge_weights.append(0.1)  # Assign a small weight for unweighted edges

        # Normalize edge weights with a minimum starting value
        norm = mcolors.Normalize(vmin=-0.5, vmax=max(edge_weights))  # Start normalization at -0.5 for better visibility
        ...
        for (parent, child) in G.edges():
            if child in terminal_nodes:
                ...
                edge_colors.append(cmap(0.4))  # Default color for non-terminal edges

        # Draw the network
        nx.draw(G, pos, ax=ax, with_labels=False, node_color=colors, node_size=800, alpha=0.6, edge_color=edge_colors)
    ...
    def show(self):
        ...
        if isinstance(self.plot, plt.Figure):  # Check if self.plot is a matplotlib figure
            plt.show()  # Display the figure in a compatible way
        else:
            raise ValueError("Plot is not a valid matplotlib Figure object. Ensure __plot_network creates a proper figure.")
    ...
    def __setstate__(self, state):
        ...
        if self.ready:  # Recreate the plot if the model is trained and ready
            self.plot = self.__plot_network()
            plt.close(self.plot)  # Close it immediately to prevent auto-rendering
